...
## 2026-07-19 / UX Planning — GitLab MR AI 초안 및 실시간 미리보기 설계
...
| 파일 | 변경 유형 | 내용 |
| --- | --- | --- |
| `docs/PRODUCT_PLAN.md` | 수정 | GitLab 저장소/MR 선택, AI 변경 요약 초안, 사용자 수정 보호, sticky preview 설계 |
| `docs/STITCH_DESIGN_BRIEF.md` | 수정 | Task Detail Editor v5 보완 프롬프트와 디자인 승인 기준 |
| `docs/PROJECT_GUIDE.md` | 수정 | Unit 9 범위와 완료 기준 추가 |
| `docs/NEXT_TASK_DRAFT.md` | 수정 | Unit 9 구현 후보, 상태 모델, 예상 변경 범위 추가 |
| `docs/SESSION_STATE.md` | 수정 | Stitch v4 리뷰 결과와 다음 UX 작업 상태 반영 |

### 작업 내용

- GitLab 연동 흐름을 `저장소 Search Select → MR Search Select → AI 분석 → 사용자 검토·수정 → 저장`으로 정의했다.
- 최초 MR 선택 시 변경 요약이 비어 있을 때만 AI 초안을 자동 입력하고, 사용자 작성 내용은 재생성이나 MR 변경으로 덮어쓰지 않도록 규칙을 정했다.
- AI 분석 중, 생성 완료, 사용자 수정, 실패, stale 상태를 명시했다.
- 데스크톱에서 편집 폼과 우측 sticky preview를 동시에 제공하고 좁은 화면에서는 편집/미리보기 탭으로 전환하도록 정의했다.
- Stitch에 전달할 Task Detail Editor v5 전용 보완 프롬프트를 작성했다.
- 외부 LLM API Key 없이 deterministic mock generator를 사용하고, 추후 사내 LLM을 서버/BFF adapter로 교체하는 연동 경계를 정의했다.
- 사내 LLM 교체 주석은 UI가 아닌 generator factory 또는 adapter 한 곳에만 남기도록 정했다.

### 검증 결과

- 문서 설계 작업으로 lint/test/typecheck/build는 실행하지 않았다.
- `CURRENT_TASK.md`는 구현 착수 요청이 없어 Unit 8 상태를 유지했다.

### 남은 리스크

- 실제 AI 모델에 전달할 MR diff 범위와 사내 코드 보안 정책이 필요하다.
- MR head SHA 기반 stale 판정과 AI 생성 API 계약을 구현 전에 확정해야 한다.
- Stitch v5 결과에서 Search Select 식별성, AI 상태, sticky preview를 다시 검수해야 한다.
- 사내 LLM endpoint의 인증, 입력 데이터 보안, timeout, audit 정책은 실제 연동 전 연구소와 합의해야 한다.

---

## 2026-07-06 / Product Planning — 현행 형상관리 문서 통합 기획 보강
...
| 파일 | 변경 유형 | 내용 |
| --- | --- | --- |
| `docs/PRODUCT_PLAN.md` | 신규 | 실제 릴리즈 노트, CHANGELOG, QC 보드 구조 기반 제품 기획안 |
| `docs/STITCH_DESIGN_BRIEF.md` | 신규 | Stitch 전달용 디자인 브리프와 프롬프트 |
| `docs/README.md` | 수정 | 신규 기획/디자인 문서 역할 추가 |

### 작업 내용

- Google Docs 릴리즈 노트, Google Sheets CHANGELOG, Google Sheets QC 보드의 공통 필드를 기준으로 ReleaseHub 원천 데이터 모델을 정리했다.
- 작업 1건을 `ReleaseTask`로 관리하고 릴리즈 노트, CHANGELOG, QC 보드를 산출물로 생성하는 구조를 명시했다.
- 이미지 첨부 문제는 문서 표 내부 삽입이 아니라 `Evidence` 자산 관리와 Before/After 비교 화면으로 해결하는 방향을 정의했다.
- Stitch 디자인 생성을 위한 화면별 요구사항, 시각 스타일, 디자인 리뷰 기준을 별도 문서로 분리했다.

### 검증 결과

- 문서 작성 작업이므로 lint/test/typecheck/build는 실행하지 않았다.
- 기존 `docs/CURRENT_TASK.md`의 Unit 1 구현 지시는 변경하지 않았다.

### 남은 리스크

- 실제 구현 모델은 기존 `PRD.mdc`의 `ReleaseItem`, `QCTestCase` 타입 초안과 이번 `ReleaseTask`, `Evidence`, `QcCase` 모델을 합쳐 SSOT로 정리해야 한다.
- Stitch 결과물 검토 후 화면 단위와 구현 Unit 계획을 재조정해야 한다.
